        ...
        <div class="card-body">
          <form action="{{ isset($blog) ? route('blogs.update', $blog->id) : route('blogs.store') }}" method="POST"
            ...
            <div class="form-group">
              <input type="text" value="{{ isset($blog) ? $blog->title : '' }}" name="title"
                class="@error('title') is-invalid @enderror form-control" placeholder="title" />
              @error('title')
            ...
            <!-- desc -->
            <div class="form-group">
              <textarea type="text" name="desc" class="@error('desc') is-invalid @enderror form-control"
                placeholder="description">{{ isset($blog) ? $blog->desc : '' }}</textarea>
              @error('desc')
              <div style="margin-top: 5px" class="alert alert-danger">
                {{ $message }}
              </div>
            ...
            <div class="form-group">
              ...
              <select class="custom-select" name="category_id">
                @foreach ($cats as $cat)
                <option value="{{ $cat->id }}" @if (isset($blog)) @if ($blog->category_id == $cat->id) selected @endif
            ...
            <div class="form-check">
              <input type="checkbox" class="form-check-input" name="tags[]" value="{{ $tag->id }}" @if (isset($blog))
